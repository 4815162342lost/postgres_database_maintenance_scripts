...
if [ "$weekends" != "yes" ]
  then
    ...
    pg_dump -p $database_port -U $database_user -h $datapase_host -Fc -f $daily_backup_dir$database_name"_"$file_name -d $database_name
    #make rsync
    #rsync -zvh $daily_backup_dir$database_name"_"$file_name $rsync_command_args
fi
...
if [ "$day_of_month" -eq "1" ]
  ...
  if [ "$weekends" != "yes" ]
    then
      ...
      cp $daily_backup_dir$database_name"_"$file_name $monthly_backup_dir
  else
    echo "Starting backup creation..."
    pg_dump -p $database_port -U $database_user -h $datapase_host -Fc -f $monthly_backup_dir$database_name"_"$file_name -d $database_name
  fi
fi
...
if [ "$day_of_year" == "001" ]
  then
    ...
    if [ "$weekends" != "yes" ]
      ...
      cp $daily_backup_dir$database_name"_"$file_name $yearly_backup_dir
    elif [ "$weekends" == "yes" ] && [ "$day_of_month" -eq "1" ]
      then
      cp $monthly_backup_dir$database_name"_"$file_name $yearly_backup_dir
    else
      echo "Starting backup creation..."
      pg_dump -p $database_port -U $database_user -h $datapase_host -Fc -f $yearly_backup_dir$database_name"_"$file_name -d $database_name
    fi
fi
